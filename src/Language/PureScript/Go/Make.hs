...
compile opts = do
  ...
  unless (null corefnFiles) $ do
    ...
    -- User foreigns, located the way purs locates `.js`: a `.go` file
    -- co-located with the `.purs`, discovered via CoreFn modulePath. For each
    -- reachable module with foreign imports, look for `<dir>/<Module>.go` next
    -- to its source. If present, copy it into the output as a sibling
    -- `package main` file `<basename>_foreign.go` and gofmt it -- `go run *.go`
    -- then resolves the user's `Module_member` symbols. This is ADDITIVE: a
    -- stdlib module (console, etc.) has no co-located `.go`, so it keeps using
    -- the embedded prelude.go shim untouched. Where no co-located file is
    -- found we emit a gap warning (could be an embedded shim, or a build break).
    let foreignMods = [ m | m <- emitMods, not (null (CoreFn.moduleForeign m)) ]
    forM_ foreignMods $ \m -> do
      let P.ModuleName mn = CoreFn.moduleName m
          srcGo  = dropExtension (CoreFn.modulePath m) <> ".go"
          destGo = outputDir opts </> (takeBaseName srcGo ++ "_foreign.go")
      exists <- doesFileExist srcGo
      if exists
        then do
          copyFile srcGo destGo
          gofmtInPlace destGo
          putStrLn $ "Copied co-located foreign: " ++ srcGo ++ " -> " ++ destGo
        else
          putStrLn $ "  WARNING (" ++ T.unpack mn ++ "): no co-located "
                  ++ T.unpack mn ++ ".go (looked at " ++ srcGo ++ ")"
                  ++ " -- relying on an embedded shim or this will fail to build"
...
assembleFile body mainCall = T.unlines
  [ "// Generated by psgo. Do not edit. Run with: go run *.go"
  , "package main"
  , ""
  , body
  , ""
